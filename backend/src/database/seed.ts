...
async function seed() {
  ...
  try {
    ...
    const careRes = await client.query(
      `INSERT INTO staff (organisation_id, home_id, email, password_hash, first_name, last_name, role)
       VALUES ($1,$2,'staff@healthark.co.uk',$3,'James','Wilson','care_staff') RETURNING id`,
    ...
    logger.info('Seed done. Logins: admin@healthark.co.uk/Admin123!@# | manager@healthark.co.uk/Manager123!@# | staff@healthark.co.uk/CareStaff123!');
  } catch (err) {
