  ...
  try {
    ...
    const adminRes = await client.query(
      `INSERT INTO staff (organisation_id, home_id, email, password_hash, first_name, last_name, role)
       VALUES ($1,$2,'admin@compcarehub.co.uk',$3,'System','Admin','group_admin') RETURNING id`,
      [ORG_ID, homeId, adminHash]
    );
    await client.query('INSERT INTO staff_onboarding (staff_id) VALUES ($1)', [adminRes.rows[0].id]);

    const mgHash = await bcrypt.hash('Manager123!@#', 12);
    const mgRes = await client.query(
      `INSERT INTO staff (organisation_id, home_id, email, password_hash, first_name, last_name, role)
       VALUES ($1,$2,'manager@compcarehub.co.uk',$3,'Sarah','Johnson','home_manager') RETURNING id`,
      [ORG_ID, homeId, mgHash]
    );
    await client.query('INSERT INTO staff_onboarding (staff_id) VALUES ($1)', [mgRes.rows[0].id]);

    const careHash = await bcrypt.hash('CareStaff123!', 12);
    const careRes = await client.query(
      `INSERT INTO staff (organisation_id, home_id, email, password_hash, first_name, last_name, role)
       VALUES ($1,$2,'staff@compcarehub.co.uk',$3,'James','Wilson','care_staff') RETURNING id`,
    ...
    logger.info('Seed done. Logins: admin@compcarehub.co.uk/Admin123!@# | manager@compcarehub.co.uk/Manager123!@# | staff@compcarehub.co.uk/CareStaff123!');
  } catch (err) {
