...
// Temporary passwords get read aloud, handwritten, or texted to staff who then
// retype them on their own device — Math.random().toString(36) can produce
// look-alike characters (l vs 1, 0 vs o) that cause exactly the "invalid email
// or password" mismatches staff keep reporting. This charset drops all of them.
const UNAMBIGUOUS_CHARS = 'ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789';
function randomUnambiguousPassword(length = 10): string {
  let out = '';
  for (let i = 0; i < length; i++) out += UNAMBIGUOUS_CHARS[Math.floor(Math.random() * UNAMBIGUOUS_CHARS.length)];
  return out + '!';
}
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { firstName, lastName, role, password, homeId,
      ...
      const rawPassword = password || randomUnambiguousPassword();
...
// DELETE /api/staff/:id — permanently delete (group_admin/admin only) or terminate (home_manager)
router.delete('/:id', requireRole('group_admin', 'admin'),
  param('id').matches(laxUuid), validateRequest,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const token = req.headers.authorization?.substring(7);
