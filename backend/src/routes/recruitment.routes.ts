...
router.post('/', requireRole(...RECRUITMENT_ROLES), [
  ...
  try {
    ...
    const rows = await query(
      ...
      [homeId, firstName, lastName, email || null, phone || null, position, appliedDate || ukDateStr(), status || 'applied', pipelineStage || status || 'applied', interviewDate || null, notes || null, dbsCheck || null, referenceCheck || null, trainingDone || false, dbsCleared || false, referencesDone || false, fullyCompliant || false, readyToStart || false, startDate || null]
    );
