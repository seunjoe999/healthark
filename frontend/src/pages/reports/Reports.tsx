    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Generate and view operational reports</p>
        </div>
        {homes.length > 1 && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {REPORT_TYPES.map(rt => (
          <button key={rt.value} onClick={() => setReportType(rt.value)}
            className={`p-4 rounded-xl border-2 text-left transition-colors ${reportType === rt.value ? 'border-purple-600 bg-purple-50' : 'border-gray-200 bg-white hover:border-purple-300'}`}>
            <p className={`text-sm font-semibold ${reportType === rt.value ? 'text-purple-900' : 'text-gray-800'}`}>{rt.label}</p>
  ...
  return (
    <div>
      <div className="grid grid-cols-4 gap-4 mb-4">
        <div className="bg-white rounded-xl border p-4 text-center"><p className="text-2xl font-bold text-gray-900">{summary.total}</p><p className="text-sm text-gray-500">Total plans</p></div>
        <div className="bg-green-50 rounded-xl border p-4 text-center"><p className="text-2xl font-bold text-green-700">{summary.current}</p><p className="text-sm text-gray-500">Current</p></div>
        <div className="bg-orange-50 rounded-xl border p-4 text-center"><p className="text-2xl font-bold text-orange-700">{summary.dueSoon}</p><p className="text-sm text-gray-500">Due soon</p></div>
        <div className="bg-red-50 rounded-xl border p-4 text-center"><p className="text-2xl font-bold text-red-700">{summary.overdue}</p><p className="text-sm text-gray-500">Overdue</p></div>
      </div>
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>{['Resident','Plan type','Review frequency','Next review','Status'].map(h => <th key={h} className="px-4 py-3 text-left font-medium text-gray-600 text-xs">{h}</th>)}</tr>
          </thead>
...
function FluidReport({ data }: { data: any[] }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-100">
          <tr>{['Resident','Date','Total (ml)','Target (ml)','Status'].map(h => <th key={h} className="px-4 py-3 text-left font-medium text-gray-600 text-xs">{h}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-50">
      ...
      {data.length === 0 && <p className="text-center text-gray-400 p-8 text-sm">No fluid records in this date range</p>}
    </div>
  )
}
...
function TrainingCompliance({ data }: { data: any[] }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-100">
          <tr>{['Staff member','Role','Course','Completed','Expiry','Status'].map(h => <th key={h} className="px-4 py-3 text-left font-medium text-gray-600 text-xs">{h}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-50">
          {data.map((r: any, i: number) => (
            <tr key={i}>
              <td className="px-4 py-3 font-medium">{r.staff_name}</td>
              <td className="px-4 py-3 capitalize text-xs text-gray-500">{(r.role || '').replace(/_/g, ' ')}</td>
      ...
      {data.length === 0 && <p className="text-center text-gray-400 p-8 text-sm">No training records found</p>}
    </div>
  )
}
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
        <p className="text-sm font-medium text-gray-700">{data.length} records</p>
      </div>
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-100">
          <tr>{cols.map(c => <th key={c} className="px-4 py-3 text-left font-medium text-gray-600 text-xs capitalize">{c.replace(/_/g, ' ')}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-50">
          {data.slice(0, 100).map((row: any, i: number) => (
            <tr key={i} className="hover:bg-gray-50">
              {cols.map(c => (
                <td key={c} className="px-4 py-3 text-gray-700 max-w-xs truncate">
