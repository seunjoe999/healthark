...
function TaskCard({ task, today, isRole, teams, priorityColor, onComplete, onEdit, onDelete, onReopen, userId, clockedIn }: {
  ...
  onReopen: (id: string) => void; userId?: string; clockedIn: boolean
}) {
...
export default function Tasks() {
  ...
  // Use the server's clock, not the device's — a recurring task generated by
  ...
  const loadStockCountStatus = async () => {
    try {
      const res = await api.get('/mar/stock-count-status', { params: { homeId: selectedHome } })
  ...
  const reopenTask = async (taskId: string) => {
    ...
    } catch (err: any) { toast.error(err?.response?.data?.error || 'Failed to reopen') }
  }
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2 items-center">
          ...
          {pageTab === 'tasks' && isRole(...TASK_CREATOR_ROLES) && (
            <Button size="sm" icon={<Send className="w-4 h-4" />} onClick={() => setAddFollowUpOpen(true)}
                ...
                <div className="space-y-3 mb-6">
                  ...
                  {pendingList.map((task: any) => (
                    <TaskCard key={task.id} task={task} today={today} isRole={isRole} teams={teams} priorityColor={priorityColor}
                      ...
                      onReopen={reopenTask} userId={user?.id} clockedIn={clockedIn} />
                  ))}
                </div>
              )}
