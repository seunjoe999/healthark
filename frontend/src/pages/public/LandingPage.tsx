...
const BLACK = '#1F2430';
const YELLOW = '#E8B130';
// White + blue pass: white replaces the peach section background, NAVY
// (the original blue) replaces purple as the accent colour — per explicit
// correction ("only change the colour, not the wording, back to white/blue").
const PEACH = '#FFFFFF';
const PURPLE = NAVY;
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map(l => (
              <button key={l.id} onClick={() => scrollTo(l.id)}
                className="px-5 py-3.5 text-lg font-bold rounded-full transition-colors hover:bg-white/10 whitespace-nowrap" style={{ color: '#FFFFFF' }}>
                {l.label}
              </button>
            ))}
            <Link to="/careers" className="px-5 py-3.5 text-lg font-bold rounded-full transition-colors hover:bg-white/10 whitespace-nowrap" style={{ color: '#FFFFFF' }}>
              Careers
            </Link>
          </div>

          <div className="hidden md:flex items-center gap-3 flex-shrink-0">
            <button onClick={() => setDemoOpen(true)} className="px-7 py-4 rounded-full text-lg font-bold transition-transform hover:-translate-y-0.5 whitespace-nowrap" style={{ background: '#FFFFFF', color: PURPLE }}>
              Demo
            </button>
            <Link to="/login" className="px-7 py-4 rounded-full text-lg font-semibold border-2 transition-colors hover:bg-white/10 whitespace-nowrap"
