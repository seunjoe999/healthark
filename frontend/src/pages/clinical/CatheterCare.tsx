...
export default function CatheterCare() {
  ...
  const handlePrint = () => {
    ...
    if (latestSetup) {
      sections.push({
        title: 'Catheter Details',
        inner: `<table class="fields">
          <tr><th>Type</th><td>${esc(latestSetup.catheter_type)}</td></tr>
          <tr><th>Size</th><td>Fr ${esc(latestSetup.cath_size)}</td></tr>
          <tr><th>Date Inserted</th><td>${latestSetup.inserted_date ? fmtDate(latestSetup.inserted_date) : '—'}</td></tr>
          <tr><th>Next Change Due</th><td>${latestSetup.change_date ? fmtDate(latestSetup.change_date) : '—'}</td></tr>
        </table>`,
      })
    }
    ...
    if (history.length) {
      const rows = history.map((h: any) => `
        ...
        <td>${esc(h.urine_colour)}${h.urine_amount_ml ? ` · ${esc(h.urine_amount_ml)}ml` : ''}
        ${(h.complications || []).length ? `<br/><strong style="color:#b91c1c">Complications:</strong> ${esc((h.complications || []).join(', '))}` : ''}
        ${(h.actions || []).length ? `<br/><strong>Actions:</strong> ${esc((h.actions || []).join(', '))}` : ''}
        ${h.notes ? `<br/><em>${esc(h.notes)}</em>` : ''}</td></tr>
      `).join('')
      sections.push({ title: 'Care Log', inner: `<table class="fields">${rows}</table>` })
    }

    if (!sections.length) sections.push({ title: 'Catheter Care Log', inner: `<p class="body-text muted">No catheter care records recorded yet for this resident.</p>` })

    const body = buildLetterheadPage({
      docTitle: 'Catheter Care Log', docSubtitle: 'Urinary catheter management and daily care record',
      docRefPrefix: 'CC', docRefId: selectedSu || '—', residentName, sections,
    })
    openLetterheadPrint(`${residentName} — Catheter Care Log`, body)
  }
