    ...
    <Modal title={`Adjust Stock — ${item.medication_name}`} onClose={onClose}>
      <div className="space-y-4">
        <div className="bg-slate-50 rounded-lg p-3 text-center">
          ...
          <p className="text-3xl font-bold text-slate-900">{item.quantity_remaining} <span className="text-sm font-normal text-slate-500">{item.unit}</span></p>
        </div>
        <Field label="Reason">
  ...
  return (
    <div className={`bg-white rounded-xl border shadow-sm p-4 flex flex-col gap-3 relative ${isExpired ? 'border-red-200' : isLow ? 'border-amber-200' : 'border-slate-100'}`}>
      ...
      <div className="flex items-end justify-between">
        <div>
          <span className={`text-2xl font-bold ${isLow ? 'text-amber-600' : 'text-slate-900'}`}>{item.quantity_remaining}</span>
