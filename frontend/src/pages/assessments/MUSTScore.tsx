...
export default function MUSTScore() {
  ...
  const printCurrent = () => {
    const h = history[0]
    if (!h) { toast.error('No saved MUST assessment to print yet'); return }
    ...
    const page = buildLetterheadPage({
      docTitle: 'MUST — Malnutrition Screening',
      docSubtitle: 'Malnutrition Universal Screening Tool (BAPEN)',
      docRefPrefix: 'MUST', docRefId: h.id, residentName,
      sections: [
        {
          title: 'Risk Summary',
          inner: `
        ...
        },
        {
          ...
          inner: `<table class="fields">
            <tr><th>BMI</th><td>${h.bmi ?? '—'} (score ${h.bmi_score})</td></tr>
            <tr><th>Height / Weight</th><td>${h.height_cm ?? '—'}cm / ${h.weight_kg ?? '—'}kg</td></tr>
            <tr><th>Unplanned Weight Loss Score</th><td>${h.weight_loss_score}</td></tr>
            <tr><th>Acute Disease Effect Score</th><td>${h.acute_disease_score}</td></tr>
          </table>`,
        },
        { title: 'Management Guidance', inner: `<p class="body-text">${letterNl(hr.guidance)}</p>` },
        ...(h.notes ? [{ title: 'Notes', inner: `<p class="body-text">${letterNl(h.notes)}</p>` }] : []),
      ],
    })
    if (!openLetterheadPrint('MUST Score', page)) toast.error('Pop-up blocked — please allow pop-ups for this site and try again')
  }
