  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'linear-gradient(135deg, #0d1526 0%, #151f35 35%, #1e2d4a 70%, #27334d 100%)' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 lg:p-16 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-96 h-96 rounded-full opacity-5" style={{ background: `radial-gradient(circle, ${GOLD}, transparent)`, transform: 'translate(-30%,-30%)' }} />
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
            className="rounded-3xl p-8" style={{ background: 'rgba(255,255,255,0.06)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <>
                ...
                <div className="mt-4 pt-5 border-t border-white/10 text-center">
                  <p className="text-slate-500 text-sm">New staff member?{' '}
                    <button onClick={() => { setMode('register'); setError('') }} className="font-semibold transition-opacity hover:opacity-80" style={{ color: GOLD }}>
                      Create account
                    </button>
                  </p>
                </div>
