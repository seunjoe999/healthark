  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="print-only">
        ...
        <div className="print-title">Other Risk Assessment</div>
        {selectedSu && (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 no-print">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold uppercase flex items-center gap-2" style={{ color: '#e8b130' }}>
            <Shield className="w-6 h-6" style={{ color: '#e8b130' }} /> OTHER RISK ASSESSMENT
          </h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-6 flex flex-wrap gap-4 items-end no-print">
        {homes.length > 1 && (
          <div className="flex-1 min-w-[180px]">
            ...
            <select className="input text-sm w-full" value={selectedHome} onChange={e => { setSelectedHome(e.target.value); setSelectedSu('') }}>
        ...
        <div className="flex-1 min-w-[220px]">
          ...
          <div className="relative">
            <select className="input text-sm w-full appearance-none pr-8" value={selectedSu} onChange={e => setSelectedSu(e.target.value)}>
        ...
        {selectedSu && (
          <div className="flex gap-2 flex-wrap">
            {assessments.length > 0 && <Button size="sm" variant="secondary" icon={<Printer className="w-4 h-4" />} onClick={printAll}>Print all</Button>}
            {canManage && (
              <Button size="sm" icon={<Plus className="w-4 h-4" />} onClick={() => { setForm({ ...BLANK_FORM, suId: selectedSu }); setCreateOpen(true) }}>
                New Risk Assessment
              </Button>
            )}
          </div>
      ...
      {selectedSu && assessments.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-6 no-print">
          <div className="max-w-sm">
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1 block">Search risk assessments</label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input className="input pl-9" placeholder="Search risk assessments..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
        </div>
      )}
            ...
            return (
              <button key={ra.id} onClick={() => setViewItem(ra)}
                ...
                <div className="flex items-center justify-between">
                  ...
                  {readIds.has(ra.id) && <span className="text-xs text-emerald-600 flex items-center gap-1"><BookOpen className="w-3 h-3" />Read</span>}
                </div>
                {(ra.next_review_date || ra.last_assessed_date) && (
                  <div className="flex flex-wrap gap-x-3 mt-2 text-xs text-slate-400">
                    {ra.next_review_date && <span>Review: {format(new Date(ra.next_review_date), 'd MMM yyyy')}</span>}
                    {ra.last_assessed_date && <span>Last assessed: {format(new Date(ra.last_assessed_date), 'd MMM yyyy')}</span>}
                  </div>
                )}
              </button>
            )
      ...
      {/* Detail modal */}
      <Modal open={!!viewItem} onClose={() => setViewItem(null)} title={viewItem?.assessment_name || ''} size="lg">
        {viewItem && (() => {
          const ra = viewItem
          const likelihood = LIKELIHOOD_OPTIONS.find(o => o.value === ra.risk_rating_option)
          return (
              ...
              <div className="flex items-center gap-2 flex-wrap pb-3 border-b border-slate-100">
                ...
                {canManage && (
                  <>
                    ...
                    <button
                      onClick={() => { setViewItem(null); openEdit(ra) }}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white text-slate-600 border border-slate-200 hover:bg-slate-100 transition-colors">
