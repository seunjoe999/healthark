...
export default function Navbar() {
  ...
  const linkClass = (href: string) =>
    `px-3 py-1.5 text-[12px] font-bold whitespace-nowrap rounded-full transition-all duration-200 ${
      isActive(href)
      ...
      <div className="max-w-[1280px] mx-auto px-4">
        <div className="flex items-center h-20 gap-2">
          ...
          <div className="hidden xl:flex items-center gap-1 flex-1 justify-center flex-wrap">
            ...
            {/* OUR SPECIALISM, HOW WE WORK, FEEDBACK */}
            {navAfter.filter(l => l.href !== '/join-us' && l.href !== '/make-a-referral').map(link => (
              <Link key={link.href} to={link.href} className={linkClass(link.href)}>
                {link.label}
              </Link>
            ))}
            <Link to="/join-us" className="px-3 py-1.5 text-[12px] font-bold whitespace-nowrap rounded-full transition-all duration-200 bg-brand-gold text-white hover:opacity-90">
              JOIN US
            </Link>
            <Link to="/make-a-referral" className="px-3 py-1.5 text-[12px] font-bold whitespace-nowrap rounded-full transition-all duration-200 bg-brand-red text-white hover:opacity-90">
              MAKE A REFERRAL
            </Link>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="hidden lg:flex items-center gap-1.5">
              ...
              <a href="https://wa.me/447508535012" target="_blank" rel="noreferrer"
                className="w-8 h-8 rounded-full flex items-center justify-center transition-opacity hover:opacity-80"
